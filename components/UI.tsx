...
import { getRoleBadge } from "../pages/Users/Profile";
import {
  Loader2,
  X,
  AlertTriangle,
...
>(
  (
    { className, isLoading, variant = "primary", children, disabled, ...props },
    ref,
  ) => {
    const variants = {
      primary:
        "bg-red-600 text-white hover:bg-red-700 focus:ring-red-500 shadow-lg shadow-red-100 active:scale-95",
      secondary:
        "bg-slate-900 dark:bg-slate-800 text-white hover:bg-black dark:hover:bg-slate-700 focus:ring-slate-700 shadow-lg shadow-slate-100 active:scale-95",
      danger:
        "bg-red-600 text-white hover:bg-red-700 focus:ring-red-500 shadow-lg shadow-red-100 active:scale-95",
      outline:
        "border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 focus:ring-slate-500 active:scale-95",
    };
...
>(({ className, label, error, ...props }, ref) => (
  <div className="w-full">
    ...
    />
    {error && (
      <p className="mt-2 text-[10px] text-red-600 font-black uppercase tracking-tighter">
        {error}
      </p>
    )}
  </div>
...
export const Badge: React.FC<{
  ...
  return (
    <span
      className={clsx(
        "inline-flex items-center px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-tighter",
        colors[color],
        className,
      )}
    >
...
export const RoleBadge: React.FC<{ role: string; className?: string }> = ({
  ...
  const badgeData = getRoleBadge({ role });
  
  if (badgeData) {
    return (
      <span
        className={clsx(
          "inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-[0.1em] border ring-1 ring-inset shadow-sm",
          badgeData.bg,
          badgeData.color,
          badgeData.color.replace('text-', 'border-'),
          className,
        )}
      >
  ...
  return (
    <span
      className={clsx(
        "inline-flex items-center px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-[0.1em] border bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700",
        className,
      )}
    >
      {role}
    ...
    <div className="fixed inset-0 bg-slate-900/40 dark:bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <Card className="w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200 bg-white dark:bg-slate-900 border-0 rounded-xl">
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className="text-xl font-black text-slate-900 dark:text-white mb-3 tracking-tighter transition-colors">
            {title}
          </h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 leading-relaxed font-medium transition-colors">
            {message}
          </p>
...
export const Toast: React.FC<{
  ...
  const styles = {
    success:
      "bg-white dark:bg-slate-900 border-green-500 text-green-700 dark:text-green-400 transition-colors",
    error:
      "bg-white dark:bg-slate-900 border-red-500 text-red-700 dark:text-red-400 transition-colors",
    info: "bg-white dark:bg-slate-900 border-blue-500 text-blue-700 dark:text-blue-400 transition-colors",
    warning:
      "bg-white dark:bg-slate-900 border-yellow-500 text-yellow-700 dark:text-yellow-400 transition-colors",
  };
  ...
  return (
    <div className="fixed top-6 right-6 z-[200] animate-in slide-in-from-right-10 duration-300">
      <Card
        className={clsx(
          "flex items-center gap-4 p-4 pr-6 border-l-4 shadow-2xl rounded-lg",
          styles[type],
        )}
      >
        <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded-full transition-colors">
          {Icons[type]}
        </div>
        ...
        <button
          onClick={onClose}
          className="p-1 text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400 transition-colors ml-4"
        >
          <X size={16} />
        </button>
      </Card>
