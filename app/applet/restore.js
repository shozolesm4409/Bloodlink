...
const anchor = `<Ban size={18} /></button>
...
const exactAnchorMatch = file.substring(file.indexOf(anchor)); // This is what is currently there!

const replacement = `<Ban size={18} /></button>
              ...
              return (
                <Card key={u.id} className="p-3 sm:p-4 rounded-[2rem] border-slate-100 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900 transition-colors">
                   ...
                   <div className="flex flex-row items-stretch gap-2 mt-2 pt-2 border-t border-slate-800/50">
                     ...
                     {/* Action Grid */}
                     <div className="grid grid-cols-4 gap-1.5 self-center flex-none w-[150px]">
                        {isSuperAdminViewer && (
                          <button 
