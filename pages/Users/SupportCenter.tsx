...
import { LifeBuoy, Lock, BookOpen, MessageSquare, PhoneCall, HelpCircle, CheckCircle, Send, ArrowLeft, Search, User as UserIcon, AlertCircle, ArrowRight, ShieldAlert, Paperclip, Smile, MoreHorizontal, MessageCircle, X, Share2, Facebook, Twitter, Instagram, Linkedin, Youtube, Github, Globe2, ExternalLink } from 'lucide-react';
...
export const SupportCenter = () => {
  ...
  return (
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-700 max-w-6xl mx-auto px-4 lg:px-0 transition-colors">
      <Toast {...toastState} onClose={hideToast} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 lg:gap-8">
        ...
        <div onClick={() => setActiveView('social-hub')} className="cursor-pointer group">
          <SupportLinkCard icon={Share2} title="Social Media" description="Connect with us on official platforms." color="green" />
        </div>

        {isStaff && (
          <div onClick={() => setActiveView('system-list')} className="cursor-pointer group">
            <SupportLinkCard icon={MessageSquare} title="Staff Support List" description="Manage active support communications." color="blue" badge={unreadCounts['SYSTEM'] || 0} />
          </div>
        )}
      ...
      <Card className="p-6 lg:p-8 border-0 shadow-2xl bg-white dark:bg-slate-900 rounded-sm overflow-hidden relative border border-slate-100 dark:border-slate-800 transition-colors">
        <h3 className="font-black text-xl lg:text-2xl mb-5 lg:mb-6 flex items-center gap-4 text-slate-900 dark:text-white relative z-10 transition-colors"><HelpCircle className="text-blue-600 dark:text-blue-400" size={32} /> Common Questions</h3>
        <div className="space-y-3 lg:space-y-4 relative z-10">
          <div className="p-4 lg:p-5 bg-slate-50 dark:bg-slate-800/50 rounded-sm border border-slate-100 dark:border-slate-800 transition-colors">
            <p className="font-black text-slate-800 dark:text-slate-200 mb-1 flex items-center gap-3 text-sm lg:text-base transition-colors"><CheckCircle size={20} className="text-green-500 flex-shrink-0" /> How do I verify my donation?</p>
            <p className="text-xs lg:text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed lg:pl-6 transition-colors">Our staff team will review and approve it within 24-48 hours after submission.</p>
          </div>
          <div className="p-4 lg:p-5 bg-slate-50 dark:bg-slate-800/50 rounded-sm border border-slate-100 dark:border-slate-800 transition-colors">
            <p className="font-black text-slate-800 dark:text-slate-200 mb-1 flex items-center gap-3 text-sm lg:text-base transition-colors"><CheckCircle size={20} className="text-green-500 flex-shrink-0" /> Is my phone number private?</p>
            <p className="text-xs lg:text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed lg:pl-6 transition-colors">Contact info is only visible to verified users with administrator permission.</p>
          </div>
        </div>
      </Card>
    </div>
