...
export const Profile = () => {
  ...
  const StatBox = ({ label, count, color, icon: Icon }: any) => (
    <div className={clsx("flex items-center justify-between p-4 rounded-2xl border bg-white dark:bg-slate-900 shadow-sm transition-colors", color)}>
  ...
  const IdentityRow = ({ label, value, icon: Icon }: any) => (
    <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 transition-colors">
       <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-500">{label}</span>
       </div>
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 dark:border-slate-800 pb-6 transition-colors">
         <div className="flex items-center gap-4">
           <div className="p-4 bg-red-600 text-white rounded-sm shadow-xl shadow-red-100 dark:shadow-red-900/20"><UserCircle size={32} /></div>
           <div>
             <h1 className="text-2xl lg:text-3xl font-black text-slate-900 dark:text-white tracking-tighter transition-colors">Profile Management</h1>
             <p className="text-slate-500 dark:text-slate-400 font-medium text-xs lg:text-sm">Update your digital identity and security settings.</p>
           </div>
         </div>
         <div className="flex gap-3 w-full md:w-auto">
            {user.hasIDCardAccess ? (
              <Button onClick={handleIDCardClick} variant="outline" className="flex-1 md:flex-none rounded-2xl border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">
                <IdCard className="mr-2" size={18} /> My ID Card
              </Button>
            ) : (
              <Button onClick={handleIDCardClick} isLoading={isRequesting} className="flex-1 md:flex-none rounded-2xl shadow-xl shadow-red-100 dark:shadow-red-900/20 bg-slate-900 dark:bg-slate-800 text-white">
                <ShieldAlert className="mr-2" size={18} /> Request ID
              </Button>
            )}
            
            {viewMode === 'view' ? (
              <Button onClick={() => setViewMode('edit')} className="flex-1 md:flex-none rounded-2xl shadow-xl shadow-red-100 dark:shadow-red-900/20">
                <Edit3 className="mr-2" size={18} /> Edit Profile
              </Button>
            ) : (
              <Button onClick={() => setViewMode('view')} variant="outline" className="flex-1 md:flex-none rounded-2xl border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">
                 ...
                 <div className="relative mb-6 inline-block">
                    <div className="w-40 h-40 mx-auto bg-white dark:bg-slate-800 rounded-sm p-2 shadow-xl border border-slate-100 dark:border-slate-700 transition-colors">
                       <div className="w-full h-full rounded-sm overflow-hidden relative border-4 border-slate-50 dark:border-slate-900 transition-colors">
                          {avatarPreview ? <img src={avatarPreview} className="w-full h-full object-cover" /> : <UserCircle size={64} className="text-slate-200 dark:text-slate-700 w-full h-full p-4" />}
                       </div>
                    </div>
                    {rank && (
                       <div className={clsx("absolute bottom-0 right-0 translate-x-1/4 w-10 h-10 rounded-xl flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-800", rank.bg, rank.color)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 animate-in fade-in slide-in-from-right-4">
           ...
           <div className="lg:col-span-1 space-y-2">
              <button 
                ...
                className={clsx("w-full flex items-center justify-between p-4 rounded-2xl transition-all", editTab === 'profile' ? "bg-red-600 text-white shadow-lg" : "bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800")}
              >
                 <div className="flex items-center gap-3"><UserCircle size={18} /><span className="font-bold text-sm">Profile</span></div>
                 <ChevronRight size={16} className={clsx(editTab === 'profile' ? "text-white" : "text-slate-300 dark:text-slate-600")} />
              </button>
              <button 
                onClick={() => setEditTab('image')} 
                className={clsx("w-full flex items-center justify-between p-4 rounded-2xl transition-all", editTab === 'image' ? "bg-red-600 text-white shadow-lg" : "bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800")}
              >
                 <div className="flex items-center gap-3"><ImageIcon size={18} /><span className="font-bold text-sm">Image Upload</span></div>
                 <ChevronRight size={16} className={clsx(editTab === 'image' ? "text-white" : "text-slate-300 dark:text-slate-600")} />
              </button>
              <button 
                onClick={() => setEditTab('password')} 
                className={clsx("w-full flex items-center justify-between p-4 rounded-2xl transition-all", editTab === 'password' ? "bg-red-600 text-white shadow-lg" : "bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800")}
              >
                 <div className="flex items-center gap-3"><Key size={18} /><span className="font-bold text-sm">Change Password</span></div>
                 <ChevronRight size={16} className={clsx(editTab === 'password' ? "text-white" : "text-slate-300 dark:text-slate-600")} />
              </button>
           </div>
           ...
           <div className="lg:col-span-3">
              {editTab === 'profile' && (
                <Card className="p-8 border-0 shadow-xl bg-white dark:bg-slate-900 rounded-[2.5rem] animate-in fade-in transition-colors border border-slate-100 dark:border-slate-800">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-8 flex items-center gap-3 transition-colors"><Upload size={16} /> Identity Details</h3>
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2"><Input label="BL ID Number (Protected)" value={user.idNumber} disabled className="opacity-50 font-mono dark:bg-slate-800" /></div>
                    <div className="md:col-span-2"><Input label="Email Address (Locked)" value={user.email} disabled className="opacity-50 dark:bg-slate-800" /></div>
                    ...
                    <div className="md:col-span-2 pt-6 flex justify-end">
                      <Button type="submit" isLoading={loading} disabled={isRestricted} className="w-full lg:w-auto px-12 py-5 rounded-2xl shadow-xl shadow-red-100 dark:shadow-red-900/20">
              ...
              {editTab === 'image' && (
                <Card className="p-8 border-0 shadow-xl bg-white dark:bg-slate-900 rounded-[2.5rem] animate-in fade-in transition-colors border border-slate-100 dark:border-slate-800">
                   ...
                   <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-8 flex items-center gap-3 transition-colors"><ImageIcon size={16} /> Avatar Upload</h3>
                   ...
                   <div className="flex flex-col items-center gap-8 mb-12">
                      <div className="w-40 h-40 bg-slate-50 dark:bg-slate-800 rounded-sm border-4 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden relative group transition-colors">
                         {avatarPreview ? <img src={avatarPreview} className="w-full h-full object-cover" /> : <UserCircle size={64} className="text-slate-300 dark:text-slate-600" />}
                      ...
                      <div className="w-full max-w-sm text-center">
                         ...
                         <p className="text-[10px] font-medium text-slate-400 dark:text-slate-500 mt-3 mb-8">Supported formats: JPG, PNG. Max size: 2MB.</p>
                      </div>

                      <div className="w-full pt-8 border-t border-slate-100 dark:border-slate-800">
                         <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-6 text-center transition-colors">Or Select Avatar Template</h4>
                            ...
                            {AVATAR_TEMPLATES.map((url, idx) => (
                              <button 
                                ...
                                className={clsx(
                                  ...
                                  avatarPreview === url ? "border-red-600 ring-2 ring-red-100 dark:ring-red-900/50" : "border-slate-100 dark:border-slate-700"
                                )}
                              >
                   ...
                   <div className="pt-8 border-t-2 border-dashed border-slate-100 dark:border-slate-800 transition-colors">
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-8 flex items-center gap-3 transition-colors"><Wallpaper size={16} /> Cover Upload</h3>
                      ...
                      <div className="flex flex-col gap-8">
                         <div className="w-full h-32 bg-slate-50 dark:bg-slate-800 rounded-sm border-4 border-dashed border-slate-200 dark:border-slate-700 overflow-hidden relative group transition-colors">
                            {coverPreview ? <img src={coverPreview} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-red-500 to-red-600 opacity-20 dark:opacity-40"></div>}
                            <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                               <Camera className="text-white" size={32} />
                            </div>
                         </div>
                         ...
                         <div className="w-full pt-8">
                            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-6 text-center transition-colors">Blood Related Templates</h4>
                               ...
                               {COVER_TEMPLATES.map((url, idx) => (
                                 <button 
                                   ...
                                   className={clsx(
                                     ...
                                     coverPreview === url ? "border-red-600 ring-2 ring-red-100 dark:ring-red-900/50" : "border-slate-100 dark:border-slate-700"
                                   )}
                                 >
              ...
              {editTab === 'password' && (
                <Card className="p-8 border-0 shadow-xl bg-white dark:bg-slate-900 rounded-[2.5rem] animate-in fade-in transition-colors border border-slate-100 dark:border-slate-800">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-6 flex items-center gap-3 transition-colors"><Lock size={16} /> Security Center Layout</h3>
                  <form onSubmit={handlePasswordChange} className="space-y-5 max-w-md mx-auto">
                    <Input label="Current PIN" name="currentPassword" type="password" required className="dark:bg-slate-800" />
                    <Input label="New PIN" name="newPassword" type="password" required className="dark:bg-slate-800" />
                    <Input label="Confirm PIN" name="confirmPassword" type="password" required className="dark:bg-slate-800" />
                    <Button type="submit" variant="secondary" className="w-full py-4 rounded-2xl shadow-xl dark:bg-slate-800 dark:hover:bg-slate-700 transition-all" isLoading={pwdLoading}>Update Credentials</Button>
                  </form>
                </Card>
              )}
           </div>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-2xl z-[200] flex flex-col items-center justify-center p-6 animate-in fade-in duration-300">
           ...
           <Card className="w-full max-w-lg mt-8 p-8 space-y-6 bg-white dark:bg-slate-900 border-0 dark:border dark:border-slate-800 rounded-sm">
              ...
              <div className="flex gap-4">
                ...
                <Button variant="outline" onClick={() => { setImageToCrop(null); setCropType(null); }} className="flex-1 py-4 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 rounded-2xl">
                  Cancel
                </Button>
              </div>
           </Card>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-2xl z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300 no-print">
           <div className="max-w-sm w-full animate-in zoom-in-95 duration-200 flex flex-col items-center gap-6">
             ...
             <div className="flex gap-4 w-full">
               ...
               <Button onClick={() => setShowCardModal(false)} variant="outline" className="flex-1 bg-white dark:bg-slate-800 rounded-2xl py-4 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700">
                 Close
               </Button>
             </div>
           </div>
