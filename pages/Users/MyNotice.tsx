...
export const MyNotice = () => {
  ...
  const TabButton = ({ type, label }: { type: string, label: string }) => (
    <button 
      ...
      className={clsx(
        ...
        activeTab === type ? "text-blue-600 border-b-2 border-blue-600" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
      )}
    >
      {label}
    </button>
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 px-4 animate-in fade-in duration-500 transition-colors">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 pt-4">
        ...
        {canPost && (
          <Button 
            ...
            className="rounded-xl px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-xs shadow-md border-0 dark:shadow-none"
          >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 mb-10 bg-white/50 dark:bg-slate-900/50 p-2 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
        <div className="relative flex-1 group w-full">
           ...
           <input 
             ...
             className="w-full pl-12 pr-4 py-2.5 bg-transparent border-0 text-sm font-medium outline-none focus:ring-0 text-slate-900 dark:text-white placeholder:text-slate-300 dark:placeholder:text-slate-700"
           />
        </div>
        <button 
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-40 opacity-20 dark:opacity-40">
                 ...
                 <div className="flex flex-col lg:flex-row items-start gap-4">
                    ...
                    <div className="flex items-center gap-3 w-full lg:w-auto">
                      <select 
                        ...
                        className="flex-1 lg:flex-none bg-slate-100 dark:bg-slate-800 border-0 rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest outline-none cursor-pointer text-slate-700 dark:text-slate-300 transition-colors"
                      >
                      ...
                      <label className="flex-1 lg:flex-none flex items-center justify-center gap-2 cursor-pointer bg-slate-50 dark:bg-slate-800 px-4 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700">
                        <input type="checkbox" checked={isPinned} onChange={e => setIsPinned(e.target.checked)} className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                        <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-400 tracking-widest">Pin</span>
                        <Pin size={14} className={clsx(isPinned ? "text-blue-600 fill-blue-600" : "text-slate-400 dark:text-slate-600")} />
                      </label>
                    </div>
                 </div>
                 ...
                 <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-1 p-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-x-auto no-scrollbar transition-colors">
                        ...
                        <div className="relative group">
                          <ToolbarButton icon={Baseline} title="Text Color" className="text-slate-700 dark:text-slate-300" />
                        ...
                        <div className="relative group">
                          <ToolbarButton icon={Highlighter} title="Highlight Color" className="text-slate-700 dark:text-slate-300" />
...
const NoteCard: React.FC<NoteCardProps> = ({ note, onEdit, onDelete, isStaff }) => (
  <Card className={clsx(
    ...
    note.pinned ? "border-blue-600 shadow-blue-50 dark:shadow-none" : "border-transparent hover:border-blue-500 dark:border-slate-800"
     ...
     {note.pinned && (
       <div className="absolute top-4 right-4 text-blue-600 bg-blue-50 dark:bg-blue-900/30 p-1.5 rounded-lg shadow-sm transition-colors">
     ...
     <div>
        <div className="flex items-center justify-between mb-5">
           <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden border border-slate-100 dark:border-slate-700 flex-shrink-0 transition-colors">
              ...
              <div className="min-w-0">
                 <p className="text-[11px] font-black text-slate-900 dark:text-white leading-none transition-colors truncate">{note.authorName}</p>
                 <p className="text-[10px] font-medium text-slate-400 dark:text-slate-500 mt-1.5 transition-colors">
           ...
           {isStaff && (
             <div className="flex items-center gap-1 lg:opacity-0 group-hover:opacity-100 transition-opacity absolute top-4 right-14">
                <button onClick={() => onEdit(note)} className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/40 text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 rounded-lg transition-colors"><Edit2 size={16}/></button>
                <button onClick={() => onDelete(note.id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/40 text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 rounded-lg transition-colors"><Trash2 size={16}/></button>
             </div>
           )}
        </div>

        <h4 className="text-[15px] font-black text-slate-800 dark:text-white mb-3 tracking-tight line-clamp-2 leading-snug transition-colors">
           {note.subject}
        </h4>
        <div 
          className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed note-preview-content h-20 transition-colors" 
          dangerouslySetInnerHTML={{ __html: note.details }} 
        />
     </div>

     <div className="mt-6 pt-4 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between transition-colors">
        <div className="flex items-center gap-2">
          <Badge color={note.type === 'PUBLIC' ? 'green' : (note.type === 'WEB' ? 'purple' : 'blue')} className="text-[8px] py-0.5 px-2 rounded-md ring-1 ring-current">
