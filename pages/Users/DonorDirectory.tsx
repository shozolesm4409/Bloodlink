  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 px-4 lg:px-0 max-w-7xl mx-auto transition-colors">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar border border-slate-100 dark:border-slate-800 transition-colors">
            ...
            <div className="h-40 relative">
              ...
              <div className="absolute -bottom-16 left-1/2 -translate-x-1/2">
                <div className={clsx(
                   "w-32 h-32 rounded-full border-[6px] shadow-xl overflow-hidden bg-white dark:bg-slate-800 transition-colors",
            ...
            <div className="pt-20 pb-8 px-8 text-center space-y-8">
              ...
              {(() => {
                ...
                return (
                  <div className={clsx("p-4 rounded-2xl border flex items-center justify-center gap-3 transition-colors", eligible ? "bg-green-50 dark:bg-green-950/20 border-green-100 dark:border-green-900/30 text-green-700 dark:text-green-400" : "bg-red-50 dark:bg-red-950/20 border-red-100 dark:border-red-900/30 text-red-700 dark:text-red-400")}>
              ...
              <div className="space-y-4 text-left">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3 transition-colors">
                      <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center text-red-600 shadow-sm shrink-0"><Droplet size={20} /></div>
                      <div className="min-w-0">
                         <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest truncate">Group</p>
                         <p className="text-lg font-black text-slate-900 dark:text-white transition-colors">{viewUser.bloodGroup}</p>
                      </div>
                   </div>
                   <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3 transition-colors">
                      <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400 shadow-sm shrink-0"><MapPin size={20} /></div>
                      <div className="min-w-0">
                         <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest truncate">Location</p>
                         <p className="text-sm font-bold text-slate-900 dark:text-white truncate transition-colors" title={viewUser.location}>{viewUser.location}</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-4 transition-colors">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Contact Number</p>
                    <a href={`tel:${viewUser.phone}`} className="text-lg font-bold text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{viewUser.phone}</a>
                  </div>
                </div>

                {viewUser.email && (
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-4 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center text-orange-600 shadow-sm shrink-0"><Mail size={20} /></div>
                    <div className="min-w-0">
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Email Address</p>
                      <p className="text-sm font-bold text-slate-900 dark:text-white truncate transition-colors">{viewUser.email}</p>
                    </div>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 text-center transition-colors">
                    <div className="w-8 h-8 mx-auto bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-lg flex items-center justify-center mb-2"><Activity size={16} /></div>
                    <p className="text-2xl font-black text-slate-900 dark:text-white transition-colors">{getDonationCount(viewUser.id)}</p>
                    <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Donations</p>
                  </div>
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 text-center transition-colors">
                    <div className="w-8 h-8 mx-auto bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 rounded-lg flex items-center justify-center mb-2"><Calendar size={16} /></div>
                    <p className="text-sm font-black text-slate-900 dark:text-white mt-2 transition-colors">{viewUser.lastDonationDate ? new Date(viewUser.lastDonationDate).toLocaleDateString() : 'N/A'}</p>
                    <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Last Donate</p>
                  </div>
                </div>
                ...
                {userFeedbacks.length > 0 && (
                  <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-4 flex items-center gap-2 transition-colors">
                      <MessageSquareQuote size={14} className="text-slate-400 dark:text-slate-500" /> Community Feedback
                    </h3>
                    <div className="space-y-3">
                      {userFeedbacks.map(f => (
                        <div key={f.id} className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 relative group transition-colors">
                          <Quote size={14} className="absolute top-3 left-3 text-slate-300 dark:text-slate-700 fill-current" />
                          <p className="text-xs font-medium text-slate-600 dark:text-slate-300 pl-5 italic leading-relaxed transition-colors">"{f.message}"</p>
                          <p className="text-[12px] font-black text-slate-400 dark:text-slate-500 text-right mt-2 uppercase tracking-widest transition-colors">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter transition-colors">Donor Directory</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium transition-colors">Find lifesavers in your local community.</p>
        </div>
        <div className="flex flex-col gap-4 w-full lg:w-auto">
          <div className="flex flex-col lg:flex-row gap-4 w-full">
            {/* Search Box - Reduced width */}
            <div className="relative group w-full lg:w-72">
               <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 group-focus-within:text-red-600 transition-colors z-10">
                 <Search size={20} />
               </div>
               <input 
                 ...
                 className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-bold outline-none shadow-xl shadow-slate-200/50 dark:shadow-none focus:ring-4 focus:ring-red-500/5 dark:focus:ring-red-500/10 focus:border-red-600 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 text-slate-900 dark:text-white"
               />
            </div>
            ...
            <div className="flex flex-row gap-4 w-full lg:w-auto">
              <div className="relative group flex-1 min-w-[140px]">
                 <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 group-focus-within:text-red-600 transition-colors z-10 pointer-events-none">
                   <MapPin size={18} />
                 </div>
                 <select 
                   value={locationFilter} 
                   onChange={e => setLocationFilter(e.target.value)} 
                   className="w-full pl-12 pr-8 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-black uppercase tracking-widest outline-none shadow-xl shadow-slate-200/50 dark:shadow-none focus:border-red-600 cursor-pointer appearance-none text-slate-700 dark:text-slate-300 transition-colors"
                 >
        ...
        {filtered.map(u => {
          ...
          return (
            <Card key={u.id} className="group overflow-hidden border-0 shadow-lg bg-white dark:bg-slate-900 rounded-[2.5rem] hover:shadow-2xl dark:hover:shadow-red-900/10 border border-slate-100 dark:border-slate-800 transition-all duration-300 flex flex-col">
              {/* Cover Image Section */}
              <div className="h-28 w-full bg-slate-100 dark:bg-slate-800 relative transition-colors">
                {u.coverImage ? (
                  <img src={u.coverImage} className="w-full h-full object-cover" alt="cover" />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800"></div>
                )}
              ...
              <div className="px-6 pb-6 flex flex-col items-center flex-1 relative">
                ...
                <div className="-mt-12 mb-4 relative">
                  <div className={clsx(
                    "w-24 h-24 rounded-full border-[5px] shadow-xl overflow-hidden bg-white dark:bg-slate-800 transition-colors",
                    ...
                    {u.avatar ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-slate-50 dark:bg-slate-900 text-slate-300 dark:text-slate-700 transition-colors"><UserIcon size={32} /></div>
                    )}
                  </div>
                  
                  {/* Blood Group Badge (Bottom Left) */}
                  <div className="absolute bottom-0 left-0 -translate-x-2 translate-y-1 w-8 h-8 bg-red-600 text-white rounded-full flex items-center justify-center text-[10px] font-black border-2 border-white dark:border-slate-800 shadow-md z-10 transition-colors">
                ...
                <div className="flex items-center gap-2 mb-6">
                  ...
                  {/* BL-ID Badge */}
                  {u.idNumber && (
                    <Badge color="purple" className="text-[10px] py-0.5 px-2 border border-purple-200 dark:border-purple-900/50 font-mono transition-colors">
                      {u.idNumber}
                    </Badge>
                  )}
                </div>
                ...
                <div className="w-full grid grid-cols-2 gap-3 mb-6">
                   <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-3 text-center border border-slate-100 dark:border-slate-800 transition-colors">
                      <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Last Donate</p>
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors">{u.lastDonationDate ? new Date(u.lastDonationDate).toLocaleDateString() : 'Never'}</p>
                   </div>
                   <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-3 text-center border border-slate-100 dark:border-slate-800 transition-colors">
                      <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Score</p>
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors">{donationCount} Pts</p>
                   </div>
                </div>

                <div className="mt-auto w-full">
                  <Button onClick={() => handleViewProfile(u)} className="w-full rounded-2xl py-4 shadow-lg shadow-red-100 dark:shadow-none bg-red-600 hover:bg-red-700 group-hover:scale-[1.02] transition-transform">
