...
export const VerifyMember = () => {
  ...
  const fetchData = async (queryValue: string) => {
    ...
    try {
      ...
      const qId = query(
        collection(db, "users"),
        where("idNumber", "==", queryValue.toUpperCase()),
      );
      ...
      if (!snap.empty) {
        ...
        const feedbackData = fSnap.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as DonationFeedback,
        );
  ...
  if (!idNumber) {
    return (
      <div
        className={clsx(
          "flex flex-col items-center justify-start transition-colors w-full",
          !isAdminView && "p-4 bg-white dark:bg-slate-950 min-h-[50vh]",
        )}
      >
        <div className="max-w-xl w-full space-y-8 animate-in fade-in duration-500">
          <div className="text-center space-y-4">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium transition-colors">
              পাবলিক ভেরিফিকেশন সিস্টেম। ডোনার আইডি অথবা ফোন নম্বর দিয়ে সত্যতা
              যাচাই করুন।
            </p>
          </div>
          ...
          <Card className="p-5 border-0 shadow-2xl rounded-sm bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 transition-colors">
            <form onSubmit={handleSearch} className="space-y-6">
              <div>
                <label className="block text-xs font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-3 ml-1 transition-colors">
                  System ID or Phone Number
                </label>
                <div className="relative group">
                  <Search
                    className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 group-focus-within:text-red-600 dark:group-focus-within:text-red-400 transition-colors"
                    size={18}
                  />
                  <input
                    type="text"
              ...
              <Button
                type="submit"
                className="w-full py-5 rounded-sm text-lg shadow-xl shadow-red-100 dark:shadow-red-900/20"
              >
                ভেরিফাই করুন
              </Button>
            </form>
      ...
      >
        <Card className="max-w-md w-full p-5 text-center border-0 shadow-2xl rounded-sm bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 transition-colors">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 transition-colors">
            প্রদত্ত আইডি বা ফোন নম্বরের সাথে কোনো নিবন্ধিত রক্তদাতা পাওয়া যায়নি।
            দয়া করে তথ্য যাচাই করে আবার চেষ্টা করুন।
          </p>
          <Button
            onClick={() => navigate(baseRoute)}
            variant="outline"
            className="w-full py-4 rounded-sm border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 transition-colors"
          >
            আবার খুঁজুন
          </Button>
        </Card>
  ...
  const rank = getRankBadge(member, undefined, donationCount);

  return (
    <div
      className={clsx(
        "animate-in fade-in zoom-in-95 duration-500 transition-colors",
        !isAdminView &&
          "p-4 bg-slate-50 dark:bg-slate-950 min-h-[50vh]",
      )}
    >
      <Card className="max-w-4xl mx-auto border-0 shadow-2xl rounded-sm bg-white dark:bg-slate-900 overflow-hidden relative border border-slate-100 dark:border-slate-800 transition-colors">
        ...
        <div className="relative z-10 px-2 sm:px-4 lg:px-6 pt-4 lg:pt-6 pb-4 lg:pb-6">
            ...
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left w-full lg:w-auto">
              <div className="w-32 h-32 lg:w-40 lg:h-40 rounded-sm border-[4px] lg:border-[6px] border-white dark:border-slate-800 shadow-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 mb-4 lg:mb-6 relative transition-colors">
                ...
                {rank && (
                  <div
                    className={clsx(
                      "absolute bottom-0 right-0 w-10 h-10 lg:w-12 lg:h-12 bg-white dark:bg-slate-900 flex items-center justify-center rounded-sm shadow-inner transition-colors",
                      rank.color,
                    )}
                  >
                    <rank.icon
              ...
              <div className="w-full">
                <h2 className="text-2xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-2 transition-colors break-words px-2 lg:px-0 flex items-center justify-center lg:justify-start gap-2">
                  {member.name}
                  {(() => {
                    const vb = getVerificationBadge(member, undefined);
                    if (vb) {
                      return (
                        <BadgeCheck
                          size={28}
                ...
                <p className="text-xs lg:text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 transition-colors">
                  Official Donor
                </p>
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-2">
                  <Badge
                    color="blue"
                    className="px-3 py-1 ring-1 ring-blue-100 dark:ring-blue-900/50 font-black"
                  >
                    {member.idNumber || "NO ID"}
                  </Badge>
                  <Badge
                    color={eligible ? "green" : "red"}
                    className="px-3 py-1 ring-1 ring-current font-black"
                  >
              ...
              <div className="grid grid-cols-2 gap-1.5 lg:gap-3">
                <div className="p-1.5 lg:p-2 rounded-sm bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center gap-1.5 lg:gap-3 transition-colors shadow-sm lg:shadow-none hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm lg:text-xl font-black text-slate-900 dark:text-white transition-colors leading-none">
                      {member.bloodGroup}
                    </p>
                  </div>
                </div>
                <div className="p-1.5 lg:p-2 rounded-sm bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center gap-1.5 lg:gap-3 transition-colors shadow-sm lg:shadow-none hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm lg:text-base font-black text-slate-900 dark:text-white truncate transition-colors leading-tight">
                      {member.location}
                    </p>
                  </div>
                </div>
                <div className="p-1.5 lg:p-2 rounded-sm bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center gap-1.5 lg:gap-3 transition-colors shadow-sm lg:shadow-none hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm lg:text-lg font-black text-slate-900 dark:text-white transition-colors leading-tight">
                      {donationCount} Times
                    </p>
                  </div>
                </div>
                <div className="p-1.5 lg:p-2 rounded-sm bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center gap-1.5 lg:gap-3 transition-colors shadow-sm lg:shadow-none hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs lg:text-base font-black text-slate-900 dark:text-white transition-colors leading-tight">
                      {member.lastDonationDate
                        ? new Date(member.lastDonationDate).toLocaleDateString()
                        : "N/A"}
                    </p>
                  </div>
                </div>
          ...
          {member.socialLinks && Object.keys(member.socialLinks).length > 0 && (
            <div className="mt-3 lg:mt-5 pt-3 lg:pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors text-left">
              <h3 className="text-[10px] lg:text-xs font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-3 flex items-center justify-center lg:justify-start gap-2 transition-colors">
            ...
            <div className="mt-3 lg:mt-5 pt-3 lg:pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-3">
                {feedbacks.slice(0, 3).map((f) => (
                  <div
                    key={f.id}
                    className="p-2 lg:p-3 bg-slate-50 dark:bg-slate-800/30 rounded-sm border border-slate-100 dark:border-slate-800 relative transition-colors shadow-sm lg:shadow-none text-left flex flex-col justify-between"
                  >
                    <div>
                      <Quote
                        size={14}
          ...
          <div className="mt-3 lg:mt-5 pt-3 lg:pt-4 border-t border-slate-100 dark:border-slate-800 text-center flex flex-col sm:flex-row justify-center gap-2">
            <Button
              onClick={() => navigate(baseRoute)}
              variant="outline"
              className="border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 rounded-sm px-6 lg:px-8 text-sm transition-colors py-3 w-full sm:w-auto"
            >
              Verify Another ID
            </Button>
            {fromAdminUsers && (
              <Button
                ...
                variant="primary"
                className="hover:bg-slate-800 bg-slate-900 dark:bg-slate-800 dark:hover:bg-slate-700 text-white rounded-sm px-6 lg:px-8 text-sm transition-colors py-3 w-full sm:w-auto mt-2 sm:mt-0"
              >
