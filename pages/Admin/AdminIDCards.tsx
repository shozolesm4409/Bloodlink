...
export const IDCardFrame = React.forwardRef<HTMLDivElement, { user: User }>(({ user }, ref) => {
  ...
  return (
    <div 
      ref={ref}
      className="id-card-container relative w-[340px] h-[550px] bg-white rounded-sm shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col font-sans select-none border border-slate-100"
    >
...
export const AdminIDCards = () => {
  ...
  const downloadAsJpg = async (userId: string, name: string) => {
    ...
    try {
      ...
      showToast("Capture failed. Please wait a second for images to load.", "error");
    }
  };

  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40 gap-4 transition-colors">
      <div className="w-14 h-14 border-[5px] border-slate-100 dark:border-slate-800 border-t-red-600 rounded-full animate-spin"></div>
      <p className="font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.3em] text-[10px] transition-colors">Processing Database...</p>
    </div>
  );
    ...
    <div className="space-y-12 pb-24 max-w-7xl mx-auto px-4 animate-in fade-in duration-700 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-24 place-items-center">
             ...
             <div className="flex flex-col items-center gap-4 no-print w-full max-w-[280px] transition-colors">
                ...
                >
                   ...
                   <div className="text-left">
                      <p className="text-[11px] font-black text-[#001f3f] dark:text-slate-200 uppercase tracking-widest leading-none mb-1 transition-colors">Download Token</p>
                      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter transition-colors">High Res JPG</p>
                   </div>
                </button>
             </div>
          </div>
        ...
        {filteredRegistry.length === 0 && (
          <div className="col-span-full py-20 text-center opacity-40 transition-colors">
             <Search size={48} className="mx-auto mb-4 text-slate-300 dark:text-slate-700" />
             <p className="font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest text-xs transition-colors">No identities found matching search criteria</p>
          </div>
        )}
      </div>
