...
export const AdminPageCustomizer = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-7xl mx-auto transition-colors">
      <Toast {...toastState} onClose={hideToast} />
      
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-200 dark:border-slate-800 pb-8 px-4 lg:px-0 transition-colors">
        <div>
        ...
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl lg:rounded-3xl shadow-inner border border-slate-200 dark:border-slate-800 w-full lg:w-auto overflow-x-auto no-scrollbar transition-colors">
        ...
        <Button onClick={handleSave} isLoading={saving} className="px-10 py-5 rounded-[1.5rem] shadow-2xl bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-600 transition-all border-0 mx-4 lg:mx-0 transition-colors">
...
const SectionCard = ({ title, icon: Icon, iconColor, children }: any) => (
  <Card className="p-8 space-y-6 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900 relative overflow-hidden group h-full transition-colors">
    ...
    <div className="flex items-center gap-4 mb-2 transition-colors">
      <div className={clsx("w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-50 dark:bg-slate-800 shadow-inner transition-colors", iconColor)}>
        <Icon size={24} />
      </div>
      <h3 className="font-black text-xl text-slate-900 dark:text-white tracking-tight uppercase transition-colors">{title}</h3>
    </div>
...
const TextArea = ({ label, value, onChange }: any) => (
  <div className="space-y-2 transition-colors">
    ...
    <textarea 
      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 text-sm font-medium text-slate-900 dark:text-white focus:ring-4 focus:ring-red-500/10 outline-none min-h-[120px] shadow-inner transition-all resize-none transition-colors" 
      value={value} 
      onChange={onChange} 
    />
  </div>
