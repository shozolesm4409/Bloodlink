...
import { collection, query, orderBy, getDocs, deleteDoc, doc, getDoc, where } from 'firebase/firestore';
import { 
...
export const AdminArchives = () => {
  ...
  const [activeTab, setActiveTab] = useState<'USERS' | 'DONATIONS' | 'FUNDING' | 'FEEDBACKS' | 'NOTICES' | 'HELP' | 'LOGS' | 'VERIFICATION' | 'EXPENSES' | 'ADS'>('USERS');
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      switch(activeTab) {
        ...
        case 'ADS': 
            const q = query(collection(db, 'ad_archives'), orderBy('deletedAt', 'desc'));
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      switch(activeTab) {
        ...
        case 'ADS':
           const docSnap = await getDocs(query(collection(db, 'ad_archives'), where('id', '==', id))); // simplified
           // Correction: The ad_archives collection stores the ad document. Let's restore it.
           // Actually, the restore logic should copy it back to original collection.
           const archiveRef = doc(db, 'ad_archives', id);
           const archiveSnap = await getDoc(archiveRef);
           if(archiveSnap.exists()){
             const adData = archiveSnap.data();
             await addDoc(collection(db, COLLECTIONS.ADVERTISEMENTS), { ...adData, deletedAt: undefined });
        ...
        <div className="w-full lg:w-52 shrink-0">
          <div className="bg-white dark:bg-slate-900 rounded-sm border border-slate-200 dark:border-slate-800 p-1.5 shadow-sm sticky top-4">
            ...
            <div className="flex lg:flex-col gap-0.5 overflow-x-auto lg:overflow-x-visible pb-1.5 lg:pb-0 custom-scrollbar">
              ...
              <TabButton tab="ADS" label="Advertisements" icon={Megaphone} />
            </div>
          </div>
        </div>
